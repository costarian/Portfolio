...
import {FaTwitter, FaTumblr,FaDownload} from 'react-icons/fa'
import {TwitterShareButton,TumblrShareButton,TwitterIcon,TumblrIcon} from 'react-share'
...
function App() {
 ...
 const screenShot = async () => {
  ...
  return (
        ...
        <button onClick={getNewQuote} id="new-quote">Generate Quote</button>
        <div className="social-icons">
          <TwitterShareButton title={quotesRandom.text} url="http://localhost:3001">
          <TwitterIcon size={32} round={true} />
          </TwitterShareButton>
          <TumblrShareButton title={quotesRandom.text} url="http://localhost:3001">
          <TumblrIcon size={32} round={true} />
            </TumblrShareButton> 
        <button id="download-icon" onClick={screenShot}> <FaDownload/> </button> 
        </div>
        </div>
        <p>by Dacosta</p>
        </div>
